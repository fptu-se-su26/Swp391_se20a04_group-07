...
// Sửa lại hàm insertStudent trong StudentDAO.java
    public boolean insertStudent(Student s) {
        // [ĐÃ BỔ SUNG]: Thêm cột status = 1 vào thẳng câu SQL để đảm bảo an toàn tuyệt đối
        String sql = "INSERT INTO students (parent_id, full_name, gender, date_of_birth, school_name, class_id, address, avatar, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
            
            // Logic cực chuẩn: Nếu tìm thấy ID thì gắn, không thấy thì để NULL (Trống)
            if (s.getParentId() > 0) {
            ...
            st.setString(8, s.getAvatar());
            
            return st.executeUpdate() > 0;
        } catch (SQLException e) { 
            // [BỔ SUNG]: Báo lỗi rõ ràng ra cửa sổ Output (Tomcat) để dễ kiểm soát
            System.out.println("====== LỖI ĐÁNH BẬT TỪ DATABASE KHI INSERT HỌC SINH ======");
...
// 1. Cập nhật hàm lấy học sinh theo Lớp (Hàm quan trọng nhất cho trang này)
    ...
    public boolean updateStudent(Student s) {
        String sql = "UPDATE students SET full_name=?, gender=?, date_of_birth=?, school_name=?, class_id=?, address=?, status=? WHERE student_id=?";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
            st.setString(1, s.getFullName());
            st.setString(2, s.getGender());
            st.setDate(3, s.getDateOfBirth());
            st.setString(4, s.getSchoolName());
            st.setInt(5, s.getClassId()); // Sử dụng class_id
            st.setString(6, s.getAddress());
            st.setBoolean(7, s.isStatus());
            st.setInt(8, s.getStudentId());
            return st.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
        }
